 ...
 * limitations under the License.
 */
package com.kkkkan.youtube.tubtub.database;
...
public class YouTubeSqlDb {
    ...
    public void init(Context context) {
        ...
        //I created a table of favorites, playlists, recently seen in the DB by .getWritableDatabese
        //.getWritableDatabeseすることでdb.SQLexe()可能になる→DBの中にお気に入り、プレイリスト、最近見たもののテーブル作成してる
        dbHelper.getWritableDatabase();

        //Make playlists a new playlist
        //playlistsを新しいプレイリストにする
        playlists = new Playlists();

        //Recently watched video, favorite video assigned to recentlyWatch etc.
    ...
    private final class YouTubeDbHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            //Recently created list creation
            //最近見たものリスト作成
    ...
    public class Videos {
        ...
        public boolean create(YouTubeVideo video) {
            ...
            if (tableName.equals(FAVORITES_TABLE_NAME) && checkIfExists(video.getId())) {
                //False if there is a specified video
                //指定したビデオがあったらfalse
                //Do not allow duplicate favorites list
                //お気に入りリストは重複許さない
                return false;
            }
            ...
            if (!result) {
                //insert error = Potential over capacity
                //insert errror時=容量オーバーの可能性
                Cursor c = null;
                try {
                    //Return COLUMN_ENTRY_ID of the oldest one in the recently seen list
        ...
        public ArrayList<YouTubeVideo> readAll() {
            ...
            Cursor c = null;
            try {
                //Return data in order of newest
                //新しいもの順にデータ返す
